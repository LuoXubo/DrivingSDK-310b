...
def _build_args(projection_mat_in__, pts_extend_in__, point_2d_out_):
    __inputs__ = []
    for arg in [projection_mat_in__, pts_extend_in__]:
        if arg != None:
...
@tbe_register.register_operator("BatchMatmulVector", trans_bool_to_s8=False)
...
def batch_matmul_vector(projection_mat_in__, pts_extend_in__, point_2d_out_, kernel_name="batch_matmul_vector", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(projection_mat_in__, pts_extend_in__, point_2d_out_)
    ...
    origin_func_name = "batch_matmul_vector"
    ascendc_src_dir_ex = "batch_matmul_vector"
    ascendc_src_dir = "batch_matmul_vector"
    ascendc_src_file = "batch_matmul_vector.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BatchMatmulVector. kernel name is " + kernel_name)
    op_type = "BatchMatmulVector"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[projection_mat_in__, pts_extend_in__], origin_outputs = [point_2d_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(projection_mat_in__, pts_extend_in__, point_2d_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(projection_mat_in__, pts_extend_in__, point_2d_out_)
    result = check_op_cap("op_select_format", "BatchMatmulVector", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(projection_mat_in__, pts_extend_in__, point_2d_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(projection_mat_in__, pts_extend_in__, point_2d_out_)
    result = check_op_cap("get_op_specific_info", "BatchMatmulVector", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
