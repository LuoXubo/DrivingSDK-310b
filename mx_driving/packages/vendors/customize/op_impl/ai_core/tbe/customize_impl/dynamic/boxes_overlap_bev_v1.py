...
def _build_args(boxes_a_in__, boxes_b_in__, res_out_, format_flag, clockwise, mode_flag, aligned, margin):
    __inputs__ = []
    for arg in [boxes_a_in__, boxes_b_in__]:
        if arg != None:
    ...
    if format_flag != None:
        attr = {}
        attr["name"] = "format_flag"
        attr["dtype"] = "int"
        attr["value"] = format_flag
...
@tbe_register.register_operator("BoxesOverlapBevV1", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.OPTION_ATTR_FLOAT, para_check.KERNEL_NAME)
def boxes_overlap_bev_v1(boxes_a_in__, boxes_b_in__, res_out_, format_flag=1, clockwise=True, mode_flag=0, aligned=False, margin=1e-05, kernel_name="boxes_overlap_bev_v1", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, res_out_, format_flag, clockwise, mode_flag, aligned, margin)
    ...
    origin_func_name = "boxes_overlap_bev_v1"
    ascendc_src_dir_ex = "boxes_overlap_bev_v1"
    ascendc_src_dir = "boxes_overlap_bev_v1"
    ascendc_src_file = "boxes_overlap_bev_v1.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BoxesOverlapBevV1. kernel name is " + kernel_name)
    op_type = "BoxesOverlapBevV1"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[boxes_a_in__, boxes_b_in__], origin_outputs = [res_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(boxes_a_in__, boxes_b_in__, res_out_, format_flag=1, clockwise=True, mode_flag=0, aligned=False, margin=1e-05, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, res_out_, format_flag, clockwise, mode_flag, aligned, margin)
    result = check_op_cap("op_select_format", "BoxesOverlapBevV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_a_in__, boxes_b_in__, res_out_, format_flag=1, clockwise=True, mode_flag=0, aligned=False, margin=1e-05, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, res_out_, format_flag, clockwise, mode_flag, aligned, margin)
    result = check_op_cap("get_op_specific_info", "BoxesOverlapBevV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
