...
def _build_args(anchors_in__, origin_pos_in__, heading_out_):
    __inputs__ = []
    for arg in [anchors_in__, origin_pos_in__]:
        if arg != None:
...
@tbe_register.register_operator("CalAnchorsHeading", trans_bool_to_s8=False)
...
def cal_anchors_heading(anchors_in__, origin_pos_in__, heading_out_, kernel_name="cal_anchors_heading", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(anchors_in__, origin_pos_in__, heading_out_)
    ...
    origin_func_name = "cal_anchors_heading"
    ascendc_src_dir_ex = "cal_anchors_heading"
    ascendc_src_dir = "cal_anchors_heading"
    ascendc_src_file = "cal_anchors_heading.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator CalAnchorsHeading. kernel name is " + kernel_name)
    op_type = "CalAnchorsHeading"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[anchors_in__, origin_pos_in__], origin_outputs = [heading_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(anchors_in__, origin_pos_in__, heading_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(anchors_in__, origin_pos_in__, heading_out_)
    result = check_op_cap("op_select_format", "CalAnchorsHeading", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(anchors_in__, origin_pos_in__, heading_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(anchors_in__, origin_pos_in__, heading_out_)
    result = check_op_cap("get_op_specific_info", "CalAnchorsHeading", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
