...
def _build_args(src_in__, count_in__, out_out_):
    __inputs__ = []
    for arg in [src_in__, count_in__]:
        if arg != None:
...
@tbe_register.register_operator("ScatterMeanDiv", trans_bool_to_s8=False)
...
def scatter_mean_div(src_in__, count_in__, out_out_, kernel_name="scatter_mean_div", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, count_in__, out_out_)
    ...
    origin_func_name = "scatter_mean_div"
    ascendc_src_dir_ex = "scatter_mean_div"
    ascendc_src_dir = "scatter_mean_div"
    ascendc_src_file = "scatter_mean_div.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ScatterMeanDiv. kernel name is " + kernel_name)
    op_type = "ScatterMeanDiv"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[src_in__, count_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(src_in__, count_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, count_in__, out_out_)
    result = check_op_cap("op_select_format", "ScatterMeanDiv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(src_in__, count_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, count_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "ScatterMeanDiv", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
