...
def _build_args(src_in__, indices_in__, var_in__, out_out_, dim):
    __inputs__ = []
...
@tbe_register.register_operator("ScatterAddV1", trans_bool_to_s8=False)
...
def scatter_add_v1(src_in__, indices_in__, var_in__, out_out_, dim, kernel_name="scatter_add_v1", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, dim)
    ...
    origin_func_name = "scatter_add_v1"
    ascendc_src_dir_ex = "scatter_add_v1"
    ascendc_src_dir = "scatter_add_v1"
    ascendc_src_file = "scatter_add_v1.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ScatterAddV1. kernel name is " + kernel_name)
    op_type = "ScatterAddV1"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[src_in__, indices_in__, var_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(src_in__, indices_in__, var_in__, out_out_, dim, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, dim)
    result = check_op_cap("op_select_format", "ScatterAddV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(src_in__, indices_in__, var_in__, out_out_, dim, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, dim)
    result = check_op_cap("get_op_specific_info", "ScatterAddV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
