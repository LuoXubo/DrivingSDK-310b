...
def _build_args(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle):
    __inputs__ = []
    for arg in [gt_boxes_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    __attrs__ = []
    if feature_map_stride != None:
        attr = {}
        attr["name"] = "feature_map_stride"
        attr["dtype"] = "int"
        attr["value"] = feature_map_stride
        __attrs__.append(attr)
    if gaussian_overlap != None:
        attr = {}
        attr["name"] = "gaussian_overlap"
        attr["dtype"] = "float"
        attr["value"] = gaussian_overlap
        __attrs__.append(attr)
    if min_radius != None:
        attr = {}
        ...
        __attrs__.append(attr)
    if num_max_objs != None:
        attr = {}
        attr["name"] = "num_max_objs"
        attr["dtype"] = "int"
        attr["value"] = num_max_objs
        __attrs__.append(attr)
    if voxel_size_x != None:
        attr = {}
        attr["name"] = "voxel_size_x"
        attr["dtype"] = "float"
        attr["value"] = voxel_size_x
        __attrs__.append(attr)
    if voxel_size_y != None:
        attr = {}
        attr["name"] = "voxel_size_y"
        attr["dtype"] = "float"
        attr["value"] = voxel_size_y
        __attrs__.append(attr)
    if pc_range_x != None:
        attr = {}
        attr["name"] = "pc_range_x"
        attr["dtype"] = "float"
        attr["value"] = pc_range_x
        __attrs__.append(attr)
    if pc_range_y != None:
        attr = {}
        attr["name"] = "pc_range_y"
        attr["dtype"] = "float"
        attr["value"] = pc_range_y
        __attrs__.append(attr)
    if feature_map_size_x != None:
        attr = {}
        attr["name"] = "feature_map_size_x"
        attr["dtype"] = "int"
        attr["value"] = feature_map_size_x
        __attrs__.append(attr)
    if feature_map_size_y != None:
        attr = {}
        attr["name"] = "feature_map_size_y"
        attr["dtype"] = "int"
        attr["value"] = feature_map_size_y
        __attrs__.append(attr)
    if norm_bbox != None:
        attr = {}
        attr["name"] = "norm_bbox"
        attr["dtype"] = "bool"
        attr["value"] = norm_bbox
        __attrs__.append(attr)
    if flip_angle != None:
        attr = {}
        attr["name"] = "flip_angle"
        attr["dtype"] = "bool"
        attr["value"] = flip_angle
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("Gaussian", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def gaussian(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle, kernel_name="gaussian", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle)
    ...
    origin_func_name = "gaussian"
    ascendc_src_dir_ex = "gaussian"
    ascendc_src_dir = "gaussian"
    ascendc_src_file = "gaussian.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Gaussian. kernel name is " + kernel_name)
    op_type = "Gaussian"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[gt_boxes_in__], origin_outputs = [center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle)
    result = check_op_cap("op_select_format", "Gaussian", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gt_boxes_in__, center_int_out_, radius_out_, mask_out_, ind_out_, ret_boxes_out_, feature_map_stride, gaussian_overlap, min_radius, num_max_objs, voxel_size_x, voxel_size_y, pc_range_x, pc_range_y, feature_map_size_x, feature_map_size_y, norm_bbox, flip_angle)
    result = check_op_cap("get_op_specific_info", "Gaussian", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
