...
def _build_args(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_):
    __inputs__ = []
    for arg in [min_idx_in__, poly_line_in__, points_in__, s_cum_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [poly_start_out_, poly_end_out_, sl_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("CalcPolyStartEndSl", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def calc_poly_start_end_sl(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_, kernel_name="calc_poly_start_end_sl", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_)
    ...
    origin_func_name = "calc_poly_start_end_sl"
    ascendc_src_dir_ex = "calc_poly_start_end_sl"
    ascendc_src_dir = "calc_poly_start_end_sl"
    ascendc_src_file = "calc_poly_start_end_sl.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator CalcPolyStartEndSl. kernel name is " + kernel_name)
    op_type = "CalcPolyStartEndSl"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[min_idx_in__, poly_line_in__, points_in__, s_cum_in__], origin_outputs = [poly_start_out_, poly_end_out_, sl_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_)
    result = check_op_cap("op_select_format", "CalcPolyStartEndSl", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(min_idx_in__, poly_line_in__, points_in__, s_cum_in__, poly_start_out_, poly_end_out_, sl_out_)
    result = check_op_cap("get_op_specific_info", "CalcPolyStartEndSl", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
