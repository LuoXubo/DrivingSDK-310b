...
def _build_args(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c):
    __inputs__ = []
    for arg in [grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__]:
        if arg != None:
...
@tbe_register.register_operator("BEVPoolGrad", trans_bool_to_s8=False)
...
def bev_pool_grad(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c, kernel_name="bev_pool_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c)
    ...
    origin_func_name = "bev_pool_grad"
    ascendc_src_dir_ex = "bev_pool_grad"
    ascendc_src_dir = "b_e_v_pool_grad"
    ascendc_src_file = "bev_pool_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BEVPoolGrad. kernel name is " + kernel_name)
    op_type = "BEVPoolGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__], origin_outputs = [grad_feat_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c)
    result = check_op_cap("op_select_format", "BEVPoolGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, grad_feat_out_, b, d, h, w, c)
    result = check_op_cap("get_op_specific_info", "BEVPoolGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
