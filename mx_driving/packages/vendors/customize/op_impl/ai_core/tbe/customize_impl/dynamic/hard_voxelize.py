...
def _build_args(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points):
    __inputs__ = []
    for arg in [points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_voxels != None:
        attr = {}
        attr["name"] = "num_voxels"
        attr["dtype"] = "int"
        attr["value"] = num_voxels
        __attrs__.append(attr)
    if max_voxels != None:
        attr = {}
        attr["name"] = "max_voxels"
        attr["dtype"] = "int"
        attr["value"] = max_voxels
        __attrs__.append(attr)
    if max_points != None:
        attr = {}
        attr["name"] = "max_points"
        attr["dtype"] = "int"
        attr["value"] = max_points
        __attrs__.append(attr)
    if num_points != None:
        attr = {}
        attr["name"] = "num_points"
        attr["dtype"] = "int"
        attr["value"] = num_points
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("HardVoxelize", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def hard_voxelize(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points, kernel_name="hard_voxelize", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points)
    ...
    origin_func_name = "hard_voxelize"
    ascendc_src_dir_ex = "hard_voxelize"
    ascendc_src_dir = "hard_voxelize"
    ascendc_src_file = "hard_voxelize.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator HardVoxelize. kernel name is " + kernel_name)
    op_type = "HardVoxelize"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__], origin_outputs = [voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points)
    result = check_op_cap("op_select_format", "HardVoxelize", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, uni_voxels_in__, argsort_voxel_idices_in__, uni_argsort_idices_in__, uni_indices_in__, voxels_out_, num_points_per_voxel_out_, sorted_uni_voxels_out_, num_voxels, max_voxels, max_points, num_points)
    result = check_op_cap("get_op_specific_info", "HardVoxelize", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
