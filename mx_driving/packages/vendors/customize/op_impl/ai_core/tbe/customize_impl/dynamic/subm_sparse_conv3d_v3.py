...
def _build_args(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key):
    __inputs__ = []
    for arg in [feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [feature_out_out_, out_indices_offset_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if in_channels != None:
        attr = {}
        attr["name"] = "in_channels"
        attr["dtype"] = "int"
        attr["value"] = in_channels
        __attrs__.append(attr)
    if out_channels != None:
        attr = {}
        attr["name"] = "out_channels"
        attr["dtype"] = "int"
        attr["value"] = out_channels
        __attrs__.append(attr)
    if out_spatial_shape != None:
        attr = {}
        attr["name"] = "out_spatial_shape"
        attr["dtype"] = "list_int"
        attr["value"] = out_spatial_shape
        __attrs__.append(attr)
    if batch_size != None:
        attr = {}
        attr["name"] = "batch_size"
        attr["dtype"] = "int"
        attr["value"] = batch_size
        __attrs__.append(attr)
    if with_key != None:
        attr = {}
        attr["name"] = "with_key"
...
@tbe_register.register_operator("SubmSparseConv3dV3", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def subm_sparse_conv3d_v3(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key, kernel_name="subm_sparse_conv3d_v3", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key)
    ...
    origin_func_name = "subm_sparse_conv3d_v3"
    ascendc_src_dir_ex = "subm_sparse_conv3d_v3"
    ascendc_src_dir = "subm_sparse_conv3d_v3"
    ascendc_src_file = "subm_sparse_conv3d_v3.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SubmSparseConv3dV3. kernel name is " + kernel_name)
    op_type = "SubmSparseConv3dV3"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__], origin_outputs = [feature_out_out_, out_indices_offset_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key)
    result = check_op_cap("op_select_format", "SubmSparseConv3dV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, weight_in__, indices_in__, indices_offset_in__, map1_in__, map2_in__, feature_out_out_, out_indices_offset_out_, kernel_size, in_channels, out_channels, out_spatial_shape, batch_size, with_key)
    result = check_op_cap("get_op_specific_info", "SubmSparseConv3dV3", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
