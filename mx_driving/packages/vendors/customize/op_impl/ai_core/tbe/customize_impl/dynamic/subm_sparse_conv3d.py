...
def _build_args(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size):
    __inputs__ = []
    for arg in [feature_in__, indices_in__, weight_in__, temp_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [feature_out_out_, indices_offset_out_, indices_pair_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if out_channel != None:
        attr = {}
        attr["name"] = "out_channel"
        attr["dtype"] = "int"
        attr["value"] = out_channel
        __attrs__.append(attr)
    if outSpatialShape != None:
        attr = {}
        attr["name"] = "outSpatialShape"
        attr["dtype"] = "list_int"
        attr["value"] = outSpatialShape
        __attrs__.append(attr)
    if batch_size != None:
        attr = {}
...
@tbe_register.register_operator("SubmSparseConv3d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def subm_sparse_conv3d(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size, kernel_name="subm_sparse_conv3d", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size)
    ...
    origin_func_name = "subm_sparse_conv3d"
    ascendc_src_dir_ex = "subm_sparse_conv3d"
    ascendc_src_dir = "subm_sparse_conv3d"
    ascendc_src_file = "subm_sparse_conv3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SubmSparseConv3d. kernel name is " + kernel_name)
    op_type = "SubmSparseConv3d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[feature_in__, indices_in__, weight_in__, temp_in__], origin_outputs = [feature_out_out_, indices_offset_out_, indices_pair_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size)
    result = check_op_cap("op_select_format", "SubmSparseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feature_in__, indices_in__, weight_in__, temp_in__, feature_out_out_, indices_offset_out_, indices_pair_out_, kernel_size, out_channel, outSpatialShape, batch_size)
    result = check_op_cap("get_op_specific_info", "SubmSparseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
