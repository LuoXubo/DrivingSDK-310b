...
def _build_args(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y):
    __inputs__ = []
    for arg in [mask_in__, cur_class_id_in__, center_int_in__, radius_in__]:
        if arg != None:
    ...
    if num_classes != None:
        attr = {}
        attr["name"] = "num_classes"
        attr["dtype"] = "int"
        attr["value"] = num_classes
...
@tbe_register.register_operator("DrawGaussianToHeatmap", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def draw_gaussian_to_heatmap(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y, kernel_name="draw_gaussian_to_heatmap", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y)
    ...
    origin_func_name = "draw_gaussian_to_heatmap"
    ascendc_src_dir_ex = "draw_gaussian_to_heatmap"
    ascendc_src_dir = "draw_gaussian_to_heatmap"
    ascendc_src_file = "draw_gaussian_to_heatmap.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DrawGaussianToHeatmap. kernel name is " + kernel_name)
    op_type = "DrawGaussianToHeatmap"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[mask_in__, cur_class_id_in__, center_int_in__, radius_in__], origin_outputs = [heatmap_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y)
    result = check_op_cap("op_select_format", "DrawGaussianToHeatmap", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, cur_class_id_in__, center_int_in__, radius_in__, heatmap_out_, num_classes, feature_map_size_x, feature_map_size_y)
    result = check_op_cap("get_op_specific_info", "DrawGaussianToHeatmap", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
