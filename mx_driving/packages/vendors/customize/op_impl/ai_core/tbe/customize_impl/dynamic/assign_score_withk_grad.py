...
def _build_args(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate):
    __inputs__ = []
    for arg in [grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [grad_scores_out_, grad_points_out_, grad_centers_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_weights != None:
        attr = {}
        attr["name"] = "num_weights"
        attr["dtype"] = "int"
        attr["value"] = num_weights
        __attrs__.append(attr)
    if num_neighbors != None:
        attr = {}
        attr["name"] = "num_neighbors"
        attr["dtype"] = "int"
        attr["value"] = num_neighbors
        __attrs__.append(attr)
    if num_features != None:
        attr = {}
        attr["name"] = "num_features"
        attr["dtype"] = "int"
        attr["value"] = num_features
        __attrs__.append(attr)
    if aggregate != None:
        attr = {}
        attr["name"] = "aggregate"
...
@tbe_register.register_operator("AssignScoreWithkGrad", trans_bool_to_s8=False)
...
def assign_score_withk_grad(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, kernel_name="assign_score_withk_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    ...
    origin_func_name = "assign_score_withk_grad"
    ascendc_src_dir_ex = "assign_score_withk_grad"
    ascendc_src_dir = "assign_score_withk_grad"
    ascendc_src_file = "assign_score_withk_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator AssignScoreWithkGrad. kernel name is " + kernel_name)
    op_type = "AssignScoreWithkGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__], origin_outputs = [grad_scores_out_, grad_points_out_, grad_centers_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    result = check_op_cap("op_select_format", "AssignScoreWithkGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, points_in__, centers_in__, scores_in__, knn_idx_in__, grad_scores_out_, grad_points_out_, grad_centers_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    result = check_op_cap("get_op_specific_info", "AssignScoreWithkGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
