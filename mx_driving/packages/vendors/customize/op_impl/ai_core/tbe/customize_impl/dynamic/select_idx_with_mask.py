...
def _build_args(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_):
    __inputs__ = []
    for arg in [poly_line_in__, min_idx_in__, pt_in__, back_idx_in__]:
        if arg != None:
...
@tbe_register.register_operator("SelectIdxWithMask", trans_bool_to_s8=False)
...
def select_idx_with_mask(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_, kernel_name="select_idx_with_mask", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_)
    ...
    origin_func_name = "select_idx_with_mask"
    ascendc_src_dir_ex = "select_idx_with_mask"
    ascendc_src_dir = "select_idx_with_mask"
    ascendc_src_file = "select_idx_with_mask.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SelectIdxWithMask. kernel name is " + kernel_name)
    op_type = "SelectIdxWithMask"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[poly_line_in__, min_idx_in__, pt_in__, back_idx_in__], origin_outputs = [out_min_idx_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_)
    result = check_op_cap("op_select_format", "SelectIdxWithMask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(poly_line_in__, min_idx_in__, pt_in__, back_idx_in__, out_min_idx_out_)
    result = check_op_cap("get_op_specific_info", "SelectIdxWithMask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
