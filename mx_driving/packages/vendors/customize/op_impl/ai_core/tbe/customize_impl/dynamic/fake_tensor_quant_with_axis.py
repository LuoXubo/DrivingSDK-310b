...
def _build_args(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range):
    __inputs__ = []
    for arg in [inputs_in__, amax_in__]:
        if arg != None:
    ...
    if is_unsigned != None:
        attr = {}
        attr["name"] = "is_unsigned"
        attr["dtype"] = "bool"
        attr["value"] = is_unsigned
        __attrs__.append(attr)
    if narrow_range != None:
        attr = {}
        attr["name"] = "narrow_range"
        attr["dtype"] = "bool"
        attr["value"] = narrow_range
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("FakeTensorQuantWithAxis", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def fake_tensor_quant_with_axis(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range, kernel_name="fake_tensor_quant_with_axis", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range)
    ...
    origin_func_name = "fake_tensor_quant_with_axis"
    ascendc_src_dir_ex = "fake_tensor_quant_with_axis"
    ascendc_src_dir = "fake_tensor_quant_with_axis"
    ascendc_src_file = "fake_tensor_quant_with_axis.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FakeTensorQuantWithAxis. kernel name is " + kernel_name)
    op_type = "FakeTensorQuantWithAxis"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[inputs_in__, amax_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range)
    result = check_op_cap("op_select_format", "FakeTensorQuantWithAxis", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, axis, num_bits, is_unsigned, narrow_range)
    result = check_op_cap("get_op_specific_info", "FakeTensorQuantWithAxis", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
