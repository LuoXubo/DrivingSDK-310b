...
def _build_args(src_in__, index_in__, softmax_result_out_, N):
    __inputs__ = []
    ...
    __outputs__ = []
    for arg in [softmax_result_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("GraphSoftmax", trans_bool_to_s8=False)
...
def graph_softmax(src_in__, index_in__, softmax_result_out_, N, kernel_name="graph_softmax", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, index_in__, softmax_result_out_, N)
    ...
    origin_func_name = "graph_softmax"
    ascendc_src_dir_ex = "graph_softmax"
    ascendc_src_dir = "graph_softmax"
    ascendc_src_file = "graph_softmax.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GraphSoftmax. kernel name is " + kernel_name)
    op_type = "GraphSoftmax"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[src_in__, index_in__], origin_outputs = [softmax_result_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(src_in__, index_in__, softmax_result_out_, N, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, index_in__, softmax_result_out_, N)
    result = check_op_cap("op_select_format", "GraphSoftmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(src_in__, index_in__, softmax_result_out_, N, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, index_in__, softmax_result_out_, N)
    result = check_op_cap("get_op_specific_info", "GraphSoftmax", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
