...
def _build_args(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_):
    __inputs__ = []
    for arg in [vertices_in__, mask_in__, num_valid_in__]:
        if arg != None:
...
@tbe_register.register_operator("DiffIouRotatedSortVertices", trans_bool_to_s8=False)
...
def diff_iou_rotated_sort_vertices(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_, kernel_name="diff_iou_rotated_sort_vertices", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_)
    ...
    origin_func_name = "diff_iou_rotated_sort_vertices"
    ascendc_src_dir_ex = "diff_iou_rotated_sort_vertices"
    ascendc_src_dir = "diff_iou_rotated_sort_vertices"
    ascendc_src_file = "diff_iou_rotated_sort_vertices.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DiffIouRotatedSortVertices. kernel name is " + kernel_name)
    op_type = "DiffIouRotatedSortVertices"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[vertices_in__, mask_in__, num_valid_in__], origin_outputs = [sortedIdx_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_)
    result = check_op_cap("op_select_format", "DiffIouRotatedSortVertices", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(vertices_in__, mask_in__, num_valid_in__, sortedIdx_out_)
    result = check_op_cap("get_op_specific_info", "DiffIouRotatedSortVertices", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
