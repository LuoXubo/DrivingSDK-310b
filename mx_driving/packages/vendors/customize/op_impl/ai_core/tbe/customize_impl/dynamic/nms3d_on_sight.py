...
def _build_args(boxes_in__, mask_out_, threshold):
    __inputs__ = []
...
@tbe_register.register_operator("Nms3dOnSight", trans_bool_to_s8=False)
...
def nms3d_on_sight(boxes_in__, mask_out_, threshold, kernel_name="nms3d_on_sight", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, threshold)
    ...
    origin_func_name = "nms3d_on_sight"
    ascendc_src_dir_ex = "nms3d_on_sight"
    ascendc_src_dir = "nms3d_on_sight"
    ascendc_src_file = "nms3d_on_sight.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Nms3dOnSight. kernel name is " + kernel_name)
    op_type = "Nms3dOnSight"
...
def op_select_format(boxes_in__, mask_out_, threshold, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, threshold)
    result = check_op_cap("op_select_format", "Nms3dOnSight", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_in__, mask_out_, threshold, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, threshold)
    result = check_op_cap("get_op_specific_info", "Nms3dOnSight", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
