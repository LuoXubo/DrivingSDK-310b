...
def _build_args(x_in__, y_in__, out_out_):
    __inputs__ = []
...
@tbe_register.register_operator("Hypot", trans_bool_to_s8=False)
...
def hypot(x_in__, y_in__, out_out_, kernel_name="hypot", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_)
    ...
    origin_func_name = "hypot"
    ascendc_src_dir_ex = "hypot"
    ascendc_src_dir = "hypot"
    ascendc_src_file = "hypot.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Hypot. kernel name is " + kernel_name)
    op_type = "Hypot"
...
def op_select_format(x_in__, y_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_)
    result = check_op_cap("op_select_format", "Hypot", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, out_out_)
    result = check_op_cap("get_op_specific_info", "Hypot", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
