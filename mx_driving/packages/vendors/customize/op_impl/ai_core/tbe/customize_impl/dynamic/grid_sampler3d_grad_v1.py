...
def _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners):
    __inputs__ = []
    for arg in [grad_in__, x_in__, grid_in__]:
        if arg != None:
    ...
    if interpolation_mode != None:
        ...
        attr["dtype"] = "int"
        attr["value"] = interpolation_mode
        __attrs__.append(attr)
    if padding_mode != None:
        attr = {}
        attr["name"] = "padding_mode"
        attr["dtype"] = "int"
        attr["value"] = padding_mode
        __attrs__.append(attr)
    if align_corners != None:
        attr = {}
        attr["name"] = "align_corners"
        attr["dtype"] = "bool"
        attr["value"] = align_corners
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("GridSampler3dGradV1", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def grid_sampler3d_grad_v1(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, kernel_name="grid_sampler3d_grad_v1", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    ...
    origin_func_name = "grid_sampler3d_grad_v1"
    ascendc_src_dir_ex = "grid_sampler3d_grad_v1"
    ascendc_src_dir = "grid_sampler3d_grad_v1"
    ascendc_src_file = "grid_sampler3d_grad_v1.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GridSampler3dGradV1. kernel name is " + kernel_name)
    op_type = "GridSampler3dGradV1"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_in__, x_in__, grid_in__], origin_outputs = [dx_out_, dgrid_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("op_select_format", "GridSampler3dGradV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_in__, x_in__, grid_in__, dx_out_, dgrid_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("get_op_specific_info", "GridSampler3dGradV1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
