...
def _build_args(points_dist_in__, nearest_temp_in__, index_out_, num_points):
    __inputs__ = []
    for arg in [points_dist_in__, nearest_temp_in__]:
        if arg != None:
...
@tbe_register.register_operator("FurthestPointSamplingWithDist", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def furthest_point_sampling_with_dist(points_dist_in__, nearest_temp_in__, index_out_, num_points, kernel_name="furthest_point_sampling_with_dist", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_dist_in__, nearest_temp_in__, index_out_, num_points)
    ...
    origin_func_name = "furthest_point_sampling_with_dist"
    ascendc_src_dir_ex = "furthest_point_sampling_with_dist"
    ascendc_src_dir = "furthest_point_sampling_with_dist"
    ascendc_src_file = "furthest_point_sampling_with_dist.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FurthestPointSamplingWithDist. kernel name is " + kernel_name)
    op_type = "FurthestPointSamplingWithDist"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_dist_in__, nearest_temp_in__], origin_outputs = [index_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_dist_in__, nearest_temp_in__, index_out_, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_dist_in__, nearest_temp_in__, index_out_, num_points)
    result = check_op_cap("op_select_format", "FurthestPointSamplingWithDist", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_dist_in__, nearest_temp_in__, index_out_, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_dist_in__, nearest_temp_in__, index_out_, num_points)
    result = check_op_cap("get_op_specific_info", "FurthestPointSamplingWithDist", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
