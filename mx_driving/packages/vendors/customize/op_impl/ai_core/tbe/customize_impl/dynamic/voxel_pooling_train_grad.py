...
def _build_args(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w):
    __inputs__ = []
    for arg in [grad_out_in__, pos_memo_in__]:
        if arg != None:
...
@tbe_register.register_operator("VoxelPoolingTrainGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def voxel_pooling_train_grad(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w, kernel_name="voxel_pooling_train_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w)
    ...
    origin_func_name = "voxel_pooling_train_grad"
    ascendc_src_dir_ex = "voxel_pooling_train_grad"
    ascendc_src_dir = "voxel_pooling_train_grad"
    ascendc_src_file = "voxel_pooling_train_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator VoxelPoolingTrainGrad. kernel name is " + kernel_name)
    op_type = "VoxelPoolingTrainGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_out_in__, pos_memo_in__], origin_outputs = [grad_features_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w)
    result = check_op_cap("op_select_format", "VoxelPoolingTrainGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, pos_memo_in__, grad_features_out_, batch_size, num_points, num_channels, h, w)
    result = check_op_cap("get_op_specific_info", "VoxelPoolingTrainGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
