...
def _build_args(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_):
    __inputs__ = []
    for arg in [voxels_in__, indices_in__, argsort_indices_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("UniqueVoxel", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def unique_voxel(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_, kernel_name="unique_voxel", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_)
    ...
    origin_func_name = "unique_voxel"
    ascendc_src_dir_ex = "unique_voxel"
    ascendc_src_dir = "unique_voxel"
    ascendc_src_file = "unique_voxel.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UniqueVoxel. kernel name is " + kernel_name)
    op_type = "UniqueVoxel"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[voxels_in__, indices_in__, argsort_indices_in__], origin_outputs = [uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_)
    result = check_op_cap("op_select_format", "UniqueVoxel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, indices_in__, argsort_indices_in__, uni_voxels_out_, uni_indices_out_, uni_argsort_indices_out_, voxel_num_out_)
    result = check_op_cap("get_op_specific_info", "UniqueVoxel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
