...
def _build_args(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z):
    __inputs__ = []
    for arg in [points_in__]:
        if arg != None:
    ...
    if coors_min_x != None:
        attr = {}
        attr["name"] = "coors_min_x"
        attr["dtype"] = "float"
        attr["value"] = coors_min_x
        __attrs__.append(attr)
    if coors_min_y != None:
        attr = {}
        attr["name"] = "coors_min_y"
        attr["dtype"] = "float"
        attr["value"] = coors_min_y
        __attrs__.append(attr)
    if coors_min_z != None:
        attr = {}
        attr["name"] = "coors_min_z"
        attr["dtype"] = "float"
        attr["value"] = coors_min_z
        __attrs__.append(attr)
    if voxel_x != None:
        attr = {}
        attr["name"] = "voxel_x"
...
@tbe_register.register_operator("DynamicVoxelization", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def dynamic_voxelization(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z, kernel_name="dynamic_voxelization", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z)
    ...
    origin_func_name = "dynamic_voxelization"
    ascendc_src_dir_ex = "dynamic_voxelization"
    ascendc_src_dir = "dynamic_voxelization"
    ascendc_src_file = "dynamic_voxelization.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DynamicVoxelization. kernel name is " + kernel_name)
    op_type = "DynamicVoxelization"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__], origin_outputs = [coors_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z)
    result = check_op_cap("op_select_format", "DynamicVoxelization", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, coors_out_, coors_min_x, coors_min_y, coors_min_z, voxel_x, voxel_y, voxel_z, grid_x, grid_y, grid_z)
    result = check_op_cap("get_op_specific_info", "DynamicVoxelization", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
