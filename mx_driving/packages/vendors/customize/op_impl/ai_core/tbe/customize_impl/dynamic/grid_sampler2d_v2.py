...
def _build_args(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners):
    __inputs__ = []
    for arg in [x_trans_in__, grid_in__]:
        if arg != None:
...
@tbe_register.register_operator("GridSampler2dV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def grid_sampler2d_v2(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners, kernel_name="grid_sampler2d_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners)
    ...
    origin_func_name = "grid_sampler2d_v2"
    ascendc_src_dir_ex = "grid_sampler2d_v2"
    ascendc_src_dir = "grid_sampler2d_v2"
    ascendc_src_file = "grid_sampler2d_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GridSampler2dV2. kernel name is " + kernel_name)
    op_type = "GridSampler2dV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_trans_in__, grid_in__], origin_outputs = [y_trans_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("op_select_format", "GridSampler2dV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_trans_in__, grid_in__, y_trans_out_, interpolation_mode, padding_mode, align_corners)
    result = check_op_cap("get_op_specific_info", "GridSampler2dV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
