...
def _build_args(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num):
    __inputs__ = []
    for arg in [index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__]:
        if arg != None:
...
@tbe_register.register_operator("GraphSoftmaxGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def graph_softmax_grad(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num, kernel_name="graph_softmax_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num)
    ...
    origin_func_name = "graph_softmax_grad"
    ascendc_src_dir_ex = "graph_softmax_grad"
    ascendc_src_dir = "graph_softmax_grad"
    ascendc_src_file = "graph_softmax_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GraphSoftmaxGrad. kernel name is " + kernel_name)
    op_type = "GraphSoftmaxGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__], origin_outputs = [src_grad_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num)
    result = check_op_cap("op_select_format", "GraphSoftmaxGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(index_in__, softmax_output_in__, grad_output_in__, reduce_sum_in__, src_grad_out_, node_num)
    result = check_op_cap("get_op_specific_info", "GraphSoftmaxGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
