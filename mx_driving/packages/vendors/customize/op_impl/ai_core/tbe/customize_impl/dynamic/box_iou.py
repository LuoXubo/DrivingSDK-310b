...
def _build_args(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag, aligned):
    __inputs__ = []
...
@tbe_register.register_operator("BoxIou", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.OPTION_ATTR_BOOL, para_check.KERNEL_NAME)
def box_iou(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag=0, aligned=False, kernel_name="box_iou", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag, aligned)
    ...
    origin_func_name = "box_iou"
    ascendc_src_dir_ex = "box_iou"
    ascendc_src_dir = "box_iou"
    ascendc_src_file = "box_iou.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BoxIou. kernel name is " + kernel_name)
    op_type = "BoxIou"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[boxes_a_in__, boxes_b_in__], origin_outputs = [ious_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag=0, aligned=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag, aligned)
    result = check_op_cap("op_select_format", "BoxIou", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag=0, aligned=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_a_in__, boxes_b_in__, ious_out_, mode_flag, aligned)
    result = check_op_cap("get_op_specific_info", "BoxIou", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
