...
def _build_args(mask_in__, keep_out_, num_out_out_):
    __inputs__ = []
    for arg in [mask_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [keep_out_, num_out_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("GatherNms3dMask", trans_bool_to_s8=False)
...
def gather_nms3d_mask(mask_in__, keep_out_, num_out_out_, kernel_name="gather_nms3d_mask", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, keep_out_, num_out_out_)
    ...
    origin_func_name = "gather_nms3d_mask"
    ascendc_src_dir_ex = "gather_nms3d_mask"
    ascendc_src_dir = "gather_nms3d_mask"
    ascendc_src_file = "gather_nms3d_mask.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GatherNms3dMask. kernel name is " + kernel_name)
    op_type = "GatherNms3dMask"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[mask_in__], origin_outputs = [keep_out_, num_out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(mask_in__, keep_out_, num_out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, keep_out_, num_out_out_)
    result = check_op_cap("op_select_format", "GatherNms3dMask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(mask_in__, keep_out_, num_out_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mask_in__, keep_out_, num_out_out_)
    result = check_op_cap("get_op_specific_info", "GatherNms3dMask", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
