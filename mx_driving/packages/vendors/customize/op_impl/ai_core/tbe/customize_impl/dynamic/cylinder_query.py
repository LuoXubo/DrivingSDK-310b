...
def _build_args(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample):
    __inputs__ = []
    for arg in [new_xyz_in__, xyz_in__, rot_in__, origin_index_in__]:
        if arg != None:
    ...
    if point_cloud_size != None:
        attr = {}
        attr["name"] = "point_cloud_size"
        attr["dtype"] = "int"
        attr["value"] = point_cloud_size
        __attrs__.append(attr)
    if query_point_size != None:
        attr = {}
        attr["name"] = "query_point_size"
        attr["dtype"] = "int"
        attr["value"] = query_point_size
        __attrs__.append(attr)
    if radius != None:
        attr = {}
        attr["name"] = "radius"
...
@tbe_register.register_operator("CylinderQuery", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def cylinder_query(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample, kernel_name="cylinder_query", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample)
    ...
    origin_func_name = "cylinder_query"
    ascendc_src_dir_ex = "cylinder_query"
    ascendc_src_dir = "cylinder_query"
    ascendc_src_file = "cylinder_query.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator CylinderQuery. kernel name is " + kernel_name)
    op_type = "CylinderQuery"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[new_xyz_in__, xyz_in__, rot_in__, origin_index_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample)
    result = check_op_cap("op_select_format", "CylinderQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(new_xyz_in__, xyz_in__, rot_in__, origin_index_in__, out_out_, batch_size, point_cloud_size, query_point_size, radius, hmin, hmax, nsample)
    result = check_op_cap("get_op_specific_info", "CylinderQuery", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
