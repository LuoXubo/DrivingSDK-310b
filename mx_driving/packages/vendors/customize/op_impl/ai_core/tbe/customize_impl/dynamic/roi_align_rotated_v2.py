...
def _build_args(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise):
    __inputs__ = []
    for arg in [input_in__, rois_in__]:
        if arg != None:
...
@tbe_register.register_operator("RoiAlignRotatedV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def roi_align_rotated_v2(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise, kernel_name="roi_align_rotated_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise)
    ...
    origin_func_name = "roi_align_rotated_v2"
    ascendc_src_dir_ex = "roi_align_rotated_v2"
    ascendc_src_dir = "roi_align_rotated_v2"
    ascendc_src_file = "roi_align_rotated_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RoiAlignRotatedV2. kernel name is " + kernel_name)
    op_type = "RoiAlignRotatedV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__, rois_in__], origin_outputs = [output_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise)
    result = check_op_cap("op_select_format", "RoiAlignRotatedV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, spatial_scale, sampling_ratio, pooled_height, pooled_width, aligned, clockwise)
    result = check_op_cap("get_op_specific_info", "RoiAlignRotatedV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
