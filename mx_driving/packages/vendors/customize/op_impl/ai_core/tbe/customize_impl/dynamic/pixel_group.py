...
def _build_args(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold):
    __inputs__ = []
    for arg in [score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [point_vector_out_, label_updated_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    __attrs__ = []
    if kernel_region_num != None:
        attr = {}
        attr["name"] = "kernel_region_num"
        attr["dtype"] = "int"
        attr["value"] = kernel_region_num
        __attrs__.append(attr)
    if distance_threshold != None:
        attr = {}
        attr["name"] = "distance_threshold"
        attr["dtype"] = "float"
        attr["value"] = distance_threshold
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("PixelGroup", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_FLOAT, para_check.KERNEL_NAME)
def pixel_group(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold, kernel_name="pixel_group", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold)
    ...
    origin_func_name = "pixel_group"
    ascendc_src_dir_ex = "pixel_group"
    ascendc_src_dir = "pixel_group"
    ascendc_src_file = "pixel_group.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator PixelGroup. kernel name is " + kernel_name)
    op_type = "PixelGroup"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__], origin_outputs = [point_vector_out_, label_updated_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold)
    result = check_op_cap("op_select_format", "PixelGroup", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(score_in__, mask_in__, embedding_in__, kernel_label_in__, kernel_contour_in__, point_vector_out_, label_updated_out_, kernel_region_num, distance_threshold)
    result = check_op_cap("get_op_specific_info", "PixelGroup", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
