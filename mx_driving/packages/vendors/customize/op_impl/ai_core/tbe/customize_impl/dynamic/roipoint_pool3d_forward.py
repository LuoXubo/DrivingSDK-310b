...
def _build_args(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points):
    __inputs__ = []
    for arg in [points_in__, point_features_in__, boxes3d_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [pooled_features_out_, pooled_empty_flag_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_sampled_points != None:
        attr = {}
        attr["name"] = "num_sampled_points"
        attr["dtype"] = "int"
        attr["value"] = num_sampled_points
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("RoipointPool3dForward", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_ATTR_INT, para_check.KERNEL_NAME)
def roipoint_pool3d_forward(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points=0, kernel_name="roipoint_pool3d_forward", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points)
    ...
    origin_func_name = "roipoint_pool3d_forward"
    ascendc_src_dir_ex = "roipoint_pool3d_forward"
    ascendc_src_dir = "roipoint_pool3d_forward"
    ascendc_src_file = "roipoint_pool3d_forward.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RoipointPool3dForward. kernel name is " + kernel_name)
    op_type = "RoipointPool3dForward"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__, point_features_in__, boxes3d_in__], origin_outputs = [pooled_features_out_, pooled_empty_flag_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points)
    result = check_op_cap("op_select_format", "RoipointPool3dForward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, point_features_in__, boxes3d_in__, pooled_features_out_, pooled_empty_flag_out_, num_sampled_points)
    result = check_op_cap("get_op_specific_info", "RoipointPool3dForward", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
