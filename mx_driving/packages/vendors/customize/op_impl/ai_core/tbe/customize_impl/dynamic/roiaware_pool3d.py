...
def _build_args(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz):
    __inputs__ = []
    for arg in [rois_in__, pts_in__, pts_feature_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if max_pts_each_voxel != None:
        attr = {}
        attr["name"] = "max_pts_each_voxel"
        attr["dtype"] = "int"
        attr["value"] = max_pts_each_voxel
        __attrs__.append(attr)
    if outx != None:
        attr = {}
        attr["name"] = "outx"
...
@tbe_register.register_operator("RoiawarePool3d", trans_bool_to_s8=False)
...
def roiaware_pool3d(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz, kernel_name="roiaware_pool3d", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz)
    ...
    origin_func_name = "roiaware_pool3d"
    ascendc_src_dir_ex = "roiaware_pool3d"
    ascendc_src_dir = "roiaware_pool3d"
    ascendc_src_file = "roiaware_pool3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RoiawarePool3d. kernel name is " + kernel_name)
    op_type = "RoiawarePool3d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[rois_in__, pts_in__, pts_feature_in__], origin_outputs = [argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz)
    result = check_op_cap("op_select_format", "RoiawarePool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(rois_in__, pts_in__, pts_feature_in__, argmax_out_, pts_idx_of_voxels_out_, pooled_features_out_, mode, max_pts_each_voxel, outx, outy, outz)
    result = check_op_cap("get_op_specific_info", "RoiawarePool3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
