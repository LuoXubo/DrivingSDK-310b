...
#if defined(MX_DRIVING_ASCEND310B)
    WaitFlag<HardEvent::MTE3_V>(ping);
    for (int32_t j = 0; j < step; j++) {
        float depthValue = depthTmp_.GetValue(depOff + j * B32_DATA_NUM_PER_BLOCK);
        Muls(out_[featOff + j * channel_], featLocal_[featOff + j * channel_], depthValue, channel_);
    }
#else
    uint64_t rsvdCnt = 0;
