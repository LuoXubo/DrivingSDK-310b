...
def _build_args(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_):
    __inputs__ = []
    for arg in [features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [sparse_value_out_, sparse_indices_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("SparseMatmul", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def sparse_matmul(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_, kernel_name="sparse_matmul", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_)
    ...
    origin_func_name = "sparse_matmul"
    ascendc_src_dir_ex = "sparse_matmul"
    ascendc_src_dir = "sparse_matmul"
    ascendc_src_file = "sparse_matmul.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SparseMatmul. kernel name is " + kernel_name)
    op_type = "SparseMatmul"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__], origin_outputs = [sparse_value_out_, sparse_indices_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_)
    result = check_op_cap("op_select_format", "SparseMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, unique_indices_offset_in__, former_sorted_indices_in__, indices_in__, sparse_value_out_, sparse_indices_out_)
    result = check_op_cap("get_op_specific_info", "SparseMatmul", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
