...
def _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_):
    __inputs__ = []
    for arg in [mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DeformableAggregationGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def deformable_aggregation_grad(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_, kernel_name="deformable_aggregation_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_)
    ...
    origin_func_name = "deformable_aggregation_grad"
    ascendc_src_dir_ex = "deformable_aggregation_grad"
    ascendc_src_dir = "deformable_aggregation_grad"
    ascendc_src_file = "deformable_aggregation_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DeformableAggregationGrad. kernel name is " + kernel_name)
    op_type = "DeformableAggregationGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__], origin_outputs = [grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_)
    result = check_op_cap("op_select_format", "DeformableAggregationGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, grad_output_in__, grad_mc_ms_feat_out_, grad_sampling_location_out_, grad_weights_out_)
    result = check_op_cap("get_op_specific_info", "DeformableAggregationGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
