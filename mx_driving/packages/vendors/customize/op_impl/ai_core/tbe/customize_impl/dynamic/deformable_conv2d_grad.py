...
def _build_args(x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__, grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_, kernel_size, stride, padding, dilation, groups, deformable_groups, modulated, with_bias):
    __inputs__ = []
    for arg in [x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DeformableConv2dGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.OPTION_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def deformable_conv2d_grad(x_in__, weight_in__, bias_in__=None, offset_in__=None, mask_in__=None, offset_output_in__=None, grad_y_in__=None, grad_x_out_=None, grad_weight_out_=None, grad_bias_out_=None, grad_offset_out_=None, grad_mask_out_=None, kernel_size=[], stride=[], padding=[], dilation=[], groups=0, deformable_groups=0, modulated=False, with_bias=False, kernel_name="deformable_conv2d_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__, grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_, kernel_size, stride, padding, dilation, groups, deformable_groups, modulated, with_bias)
    ...
    origin_func_name = "deformable_conv2d_grad"
    ascendc_src_dir_ex = "deformable_conv2d_grad"
    ascendc_src_dir = "deformable_conv2d_grad"
    ascendc_src_file = "deformable_conv2d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DeformableConv2dGrad. kernel name is " + kernel_name)
    op_type = "DeformableConv2dGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__], origin_outputs = [grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, weight_in__, bias_in__=None, offset_in__=None, mask_in__=None, offset_output_in__=None, grad_y_in__=None, grad_x_out_=None, grad_weight_out_=None, grad_bias_out_=None, grad_offset_out_=None, grad_mask_out_=None, kernel_size=[], stride=[], padding=[], dilation=[], groups=0, deformable_groups=0, modulated=False, with_bias=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__, grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_, kernel_size, stride, padding, dilation, groups, deformable_groups, modulated, with_bias)
    result = check_op_cap("op_select_format", "DeformableConv2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, weight_in__, bias_in__=None, offset_in__=None, mask_in__=None, offset_output_in__=None, grad_y_in__=None, grad_x_out_=None, grad_weight_out_=None, grad_bias_out_=None, grad_offset_out_=None, grad_mask_out_=None, kernel_size=[], stride=[], padding=[], dilation=[], groups=0, deformable_groups=0, modulated=False, with_bias=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, weight_in__, bias_in__, offset_in__, mask_in__, offset_output_in__, grad_y_in__, grad_x_out_, grad_weight_out_, grad_bias_out_, grad_offset_out_, grad_mask_out_, kernel_size, stride, padding, dilation, groups, deformable_groups, modulated, with_bias)
    result = check_op_cap("get_op_specific_info", "DeformableConv2dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
