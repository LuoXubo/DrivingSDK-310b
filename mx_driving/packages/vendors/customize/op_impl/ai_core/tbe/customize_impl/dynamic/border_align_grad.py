...
def _build_args(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize):
    __inputs__ = []
    for arg in [gradOut_in__, boxes_in__, argmaxIdx_in__]:
        if arg != None:
...
@tbe_register.register_operator("BorderAlignGrad", trans_bool_to_s8=False)
...
def border_align_grad(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize, kernel_name="border_align_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize)
    ...
    origin_func_name = "border_align_grad"
    ascendc_src_dir_ex = "border_align_grad"
    ascendc_src_dir = "border_align_grad"
    ascendc_src_file = "border_align_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BorderAlignGrad. kernel name is " + kernel_name)
    op_type = "BorderAlignGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[gradOut_in__, boxes_in__, argmaxIdx_in__], origin_outputs = [gradInput_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize)
    result = check_op_cap("op_select_format", "BorderAlignGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(gradOut_in__, boxes_in__, argmaxIdx_in__, gradInput_out_, channels, boxSize, height, width, poolSize, batchSize)
    result = check_op_cap("get_op_specific_info", "BorderAlignGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
