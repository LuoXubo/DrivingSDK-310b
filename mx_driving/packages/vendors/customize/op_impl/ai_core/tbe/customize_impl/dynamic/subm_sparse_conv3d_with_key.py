...
def _build_args(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel):
    __inputs__ = []
    for arg in [outidx_offset_in__, valid_indices_in__, grad_out_features_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [grad_out_features_iml2col_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if in_channel != None:
        attr = {}
        attr["name"] = "in_channel"
        attr["dtype"] = "int"
        attr["value"] = in_channel
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("SubmSparseConv3dWithKey", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def subm_sparse_conv3d_with_key(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel, kernel_name="subm_sparse_conv3d_with_key", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel)
    ...
    origin_func_name = "subm_sparse_conv3d_with_key"
    ascendc_src_dir_ex = "subm_sparse_conv3d_with_key"
    ascendc_src_dir = "subm_sparse_conv3d_with_key"
    ascendc_src_file = "subm_sparse_conv3d_with_key.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SubmSparseConv3dWithKey. kernel name is " + kernel_name)
    op_type = "SubmSparseConv3dWithKey"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[outidx_offset_in__, valid_indices_in__, grad_out_features_in__], origin_outputs = [grad_out_features_iml2col_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel)
    result = check_op_cap("op_select_format", "SubmSparseConv3dWithKey", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(outidx_offset_in__, valid_indices_in__, grad_out_features_in__, grad_out_features_iml2col_out_, kernel_size, in_channel)
    result = check_op_cap("get_op_specific_info", "SubmSparseConv3dWithKey", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
