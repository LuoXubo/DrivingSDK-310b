...
def _build_args(point_xyz_in__, nearest_temp_in__, index_out_, num_points):
    __inputs__ = []
    for arg in [point_xyz_in__, nearest_temp_in__]:
        if arg != None:
...
@tbe_register.register_operator("FurthestPointSampling", trans_bool_to_s8=False)
...
def furthest_point_sampling(point_xyz_in__, nearest_temp_in__, index_out_, num_points, kernel_name="furthest_point_sampling", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(point_xyz_in__, nearest_temp_in__, index_out_, num_points)
    ...
    origin_func_name = "furthest_point_sampling"
    ascendc_src_dir_ex = "furthest_point_sampling"
    ascendc_src_dir = "furthest_point_sampling"
    ascendc_src_file = "furthest_point_sampling.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FurthestPointSampling. kernel name is " + kernel_name)
    op_type = "FurthestPointSampling"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[point_xyz_in__, nearest_temp_in__], origin_outputs = [index_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(point_xyz_in__, nearest_temp_in__, index_out_, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(point_xyz_in__, nearest_temp_in__, index_out_, num_points)
    result = check_op_cap("op_select_format", "FurthestPointSampling", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(point_xyz_in__, nearest_temp_in__, index_out_, num_points, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(point_xyz_in__, nearest_temp_in__, index_out_, num_points)
    result = check_op_cap("get_op_specific_info", "FurthestPointSampling", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
