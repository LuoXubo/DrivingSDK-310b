...
def _build_args(input_in__, output_out_, uniqueCnt_out_):
    __inputs__ = []
    for arg in [input_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [output_out_, uniqueCnt_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("UniqueV2", trans_bool_to_s8=False)
...
def unique_v2(input_in__, output_out_, uniqueCnt_out_, kernel_name="unique_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, uniqueCnt_out_)
    ...
    origin_func_name = "unique_v2"
    ascendc_src_dir_ex = "unique_v2"
    ascendc_src_dir = "unique_v2"
    ascendc_src_file = "unique_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator UniqueV2. kernel name is " + kernel_name)
    op_type = "UniqueV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[input_in__], origin_outputs = [output_out_, uniqueCnt_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(input_in__, output_out_, uniqueCnt_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, uniqueCnt_out_)
    result = check_op_cap("op_select_format", "UniqueV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, output_out_, uniqueCnt_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, output_out_, uniqueCnt_out_)
    result = check_op_cap("get_op_specific_info", "UniqueV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
