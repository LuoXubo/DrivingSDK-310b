...
def _build_args(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors):
    __inputs__ = []
    for arg in [x_in__, y_in__, ptr_x_in__, ptr_y_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [out_temp_out_, out_final_out_, actual_num_neighbors_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if max_num_neighbors != None:
        attr = {}
        attr["name"] = "max_num_neighbors"
        attr["dtype"] = "int"
        attr["value"] = max_num_neighbors
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("Radius", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def radius(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors, kernel_name="radius", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors)
    ...
    origin_func_name = "radius"
    ascendc_src_dir_ex = "radius"
    ascendc_src_dir = "radius"
    ascendc_src_file = "radius.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Radius. kernel name is " + kernel_name)
    op_type = "Radius"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__, ptr_x_in__, ptr_y_in__], origin_outputs = [out_temp_out_, out_final_out_, actual_num_neighbors_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors)
    result = check_op_cap("op_select_format", "Radius", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, ptr_x_in__, ptr_y_in__, out_temp_out_, out_final_out_, actual_num_neighbors_out_, r, max_num_neighbors)
    result = check_op_cap("get_op_specific_info", "Radius", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
