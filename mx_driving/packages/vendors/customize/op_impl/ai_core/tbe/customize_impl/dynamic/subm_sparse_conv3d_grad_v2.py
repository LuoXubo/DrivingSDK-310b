...
def _build_args(features_in__, weight_in__, grad_out_features_in__, indices_offset_in__, features_grad_out_, weight_grad_out_):
    __inputs__ = []
    for arg in [features_in__, weight_in__, grad_out_features_in__, indices_offset_in__]:
        if arg != None:
...
@tbe_register.register_operator("SubmSparseConv3dGradV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def subm_sparse_conv3d_grad_v2(features_in__, weight_in__, grad_out_features_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, kernel_name="subm_sparse_conv3d_grad_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, indices_offset_in__, features_grad_out_, weight_grad_out_)
    ...
    origin_func_name = "subm_sparse_conv3d_grad_v2"
    ascendc_src_dir_ex = "subm_sparse_conv3d_grad_v2"
    ascendc_src_dir = "subm_sparse_conv3d_grad_v2"
    ascendc_src_file = "subm_sparse_conv3d_grad_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SubmSparseConv3dGradV2. kernel name is " + kernel_name)
    op_type = "SubmSparseConv3dGradV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[features_in__, weight_in__, grad_out_features_in__, indices_offset_in__], origin_outputs = [features_grad_out_, weight_grad_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(features_in__, weight_in__, grad_out_features_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, indices_offset_in__, features_grad_out_, weight_grad_out_)
    result = check_op_cap("op_select_format", "SubmSparseConv3dGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(features_in__, weight_in__, grad_out_features_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, indices_offset_in__, features_grad_out_, weight_grad_out_)
    result = check_op_cap("get_op_specific_info", "SubmSparseConv3dGradV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
