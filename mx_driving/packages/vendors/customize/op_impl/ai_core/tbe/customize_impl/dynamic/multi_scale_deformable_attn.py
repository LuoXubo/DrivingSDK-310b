...
def _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_):
    __inputs__ = []
    for arg in [value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__]:
        if arg != None:
...
@tbe_register.register_operator("MultiScaleDeformableAttn", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def multi_scale_deformable_attn(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, kernel_name="multi_scale_deformable_attn", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    ...
    origin_func_name = "multi_scale_deformable_attn"
    ascendc_src_dir_ex = "multi_scale_deformable_attn"
    ascendc_src_dir = "multi_scale_deformable_attn"
    ascendc_src_file = "multi_scale_deformable_attn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MultiScaleDeformableAttn. kernel name is " + kernel_name)
    op_type = "MultiScaleDeformableAttn"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__], origin_outputs = [output_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("op_select_format", "MultiScaleDeformableAttn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, value_spatial_shapes_in__, value_level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("get_op_specific_info", "MultiScaleDeformableAttn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
