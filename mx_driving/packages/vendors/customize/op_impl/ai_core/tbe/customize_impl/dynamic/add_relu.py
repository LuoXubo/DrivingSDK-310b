...
def _build_args(x_in__, y_in__, x_out_):
    __inputs__ = []
...
@tbe_register.register_operator("AddRelu", trans_bool_to_s8=False)
...
def add_relu(x_in__, y_in__, x_out_, kernel_name="add_relu", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, x_out_)
    ...
    origin_func_name = "add_relu"
    ascendc_src_dir_ex = "add_relu"
    ascendc_src_dir = "add_relu"
    ascendc_src_file = "add_relu.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator AddRelu. kernel name is " + kernel_name)
    op_type = "AddRelu"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__], origin_outputs = [x_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, y_in__, x_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, x_out_)
    result = check_op_cap("op_select_format", "AddRelu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, x_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, x_out_)
    result = check_op_cap("get_op_specific_info", "AddRelu", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
