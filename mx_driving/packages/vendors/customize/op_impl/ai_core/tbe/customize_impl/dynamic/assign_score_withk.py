...
def _build_args(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate):
    __inputs__ = []
    for arg in [points_in__, centers_in__, scores_in__, knn_idx_in__]:
        if arg != None:
...
@tbe_register.register_operator("AssignScoreWithk", trans_bool_to_s8=False)
...
def assign_score_withk(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, kernel_name="assign_score_withk", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    ...
    origin_func_name = "assign_score_withk"
    ascendc_src_dir_ex = "assign_score_withk"
    ascendc_src_dir = "assign_score_withk"
    ascendc_src_file = "assign_score_withk.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator AssignScoreWithk. kernel name is " + kernel_name)
    op_type = "AssignScoreWithk"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__, centers_in__, scores_in__, knn_idx_in__], origin_outputs = [output_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    result = check_op_cap("op_select_format", "AssignScoreWithk", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, centers_in__, scores_in__, knn_idx_in__, output_out_, batch_size, nsource, npoint, num_weights, num_neighbors, num_features, aggregate)
    result = check_op_cap("get_op_specific_info", "AssignScoreWithk", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
