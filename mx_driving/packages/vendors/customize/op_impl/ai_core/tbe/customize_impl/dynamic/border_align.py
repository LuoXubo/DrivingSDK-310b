...
def _build_args(input_in__, rois_in__, output_out_, pooledSize):
    __inputs__ = []
    ...
    if pooledSize != None:
        attr = {}
        attr["name"] = "pooledSize"
        attr["dtype"] = "int"
        attr["value"] = pooledSize
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("BorderAlign", trans_bool_to_s8=False)
...
def border_align(input_in__, rois_in__, output_out_, pooledSize, kernel_name="border_align", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, pooledSize)
    ...
    origin_func_name = "border_align"
    ascendc_src_dir_ex = "border_align"
    ascendc_src_dir = "border_align"
    ascendc_src_file = "border_align.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BorderAlign. kernel name is " + kernel_name)
    op_type = "BorderAlign"
...
def op_select_format(input_in__, rois_in__, output_out_, pooledSize, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, pooledSize)
    result = check_op_cap("op_select_format", "BorderAlign", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(input_in__, rois_in__, output_out_, pooledSize, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(input_in__, rois_in__, output_out_, pooledSize)
    result = check_op_cap("get_op_specific_info", "BorderAlign", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
