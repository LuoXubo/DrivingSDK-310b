...
def _build_args(boxes_in__, mask_out_, nms_overlap_thresh):
    __inputs__ = []
    for arg in [boxes_in__]:
        if arg != None:
    ...
    __attrs__ = []
    if nms_overlap_thresh != None:
        attr = {}
        attr["name"] = "nms_overlap_thresh"
        attr["dtype"] = "float"
        attr["value"] = nms_overlap_thresh
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("Nms3dNormal", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.KERNEL_NAME)
def nms3d_normal(boxes_in__, mask_out_, nms_overlap_thresh, kernel_name="nms3d_normal", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, nms_overlap_thresh)
    ...
    origin_func_name = "nms3d_normal"
    ascendc_src_dir_ex = "nms3d_normal"
    ascendc_src_dir = "nms3d_normal"
    ascendc_src_file = "nms3d_normal.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Nms3dNormal. kernel name is " + kernel_name)
    op_type = "Nms3dNormal"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[boxes_in__], origin_outputs = [mask_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(boxes_in__, mask_out_, nms_overlap_thresh, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, nms_overlap_thresh)
    result = check_op_cap("op_select_format", "Nms3dNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_in__, mask_out_, nms_overlap_thresh, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, mask_out_, nms_overlap_thresh)
    result = check_op_cap("get_op_specific_info", "Nms3dNormal", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
