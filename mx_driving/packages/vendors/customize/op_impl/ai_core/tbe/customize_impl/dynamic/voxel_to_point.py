...
def _build_args(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout):
    __inputs__ = []
    for arg in [voxels_in__]:
        if arg != None:
...
@tbe_register.register_operator("VoxelToPoint", trans_bool_to_s8=False)
...
def voxel_to_point(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout, kernel_name="voxel_to_point", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout)
    ...
    origin_func_name = "voxel_to_point"
    ascendc_src_dir_ex = "voxel_to_point"
    ascendc_src_dir = "voxel_to_point"
    ascendc_src_file = "voxel_to_point.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator VoxelToPoint. kernel name is " + kernel_name)
    op_type = "VoxelToPoint"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[voxels_in__], origin_outputs = [points_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout)
    result = check_op_cap("op_select_format", "VoxelToPoint", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(voxels_in__, points_out_, voxel_sizes, coor_ranges, layout)
    result = check_op_cap("get_op_specific_info", "VoxelToPoint", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
