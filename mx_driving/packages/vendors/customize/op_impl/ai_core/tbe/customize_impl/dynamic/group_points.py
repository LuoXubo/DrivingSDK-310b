...
def _build_args(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample):
    __inputs__ = []
    for arg in [points_in__, indices_in__]:
        if arg != None:
...
@tbe_register.register_operator("GroupPoints", trans_bool_to_s8=False)
...
def group_points(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample, kernel_name="group_points", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample)
    ...
    origin_func_name = "group_points"
    ascendc_src_dir_ex = "group_points"
    ascendc_src_dir = "group_points"
    ascendc_src_file = "group_points.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GroupPoints. kernel name is " + kernel_name)
    op_type = "GroupPoints"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__, indices_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample)
    result = check_op_cap("op_select_format", "GroupPoints", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, indices_in__, out_out_, b, c, n, npoints, nsample)
    result = check_op_cap("get_op_specific_info", "GroupPoints", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
