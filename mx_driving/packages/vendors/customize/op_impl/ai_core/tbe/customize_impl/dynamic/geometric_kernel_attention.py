...
def _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_):
    __inputs__ = []
    for arg in [value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__]:
        if arg != None:
...
@tbe_register.register_operator("GeometricKernelAttention", trans_bool_to_s8=False)
...
def geometric_kernel_attention(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, kernel_name="geometric_kernel_attention", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    ...
    origin_func_name = "geometric_kernel_attention"
    ascendc_src_dir_ex = "geometric_kernel_attention"
    ascendc_src_dir = "geometric_kernel_attention"
    ascendc_src_file = "geometric_kernel_attention.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator GeometricKernelAttention. kernel name is " + kernel_name)
    op_type = "GeometricKernelAttention"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__], origin_outputs = [output_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("op_select_format", "GeometricKernelAttention", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_locations_in__, attention_weights_in__, output_out_)
    result = check_op_cap("get_op_specific_info", "GeometricKernelAttention", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
