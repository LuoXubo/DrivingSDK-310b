...
def _build_args(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels):
    __inputs__ = []
    for arg in [features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [output_img2col_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("SparseInverseConv3d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def sparse_inverse_conv3d(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels, kernel_name="sparse_inverse_conv3d", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels)
    ...
    origin_func_name = "sparse_inverse_conv3d"
    ascendc_src_dir_ex = "sparse_inverse_conv3d"
    ascendc_src_dir = "sparse_inverse_conv3d"
    ascendc_src_file = "sparse_inverse_conv3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SparseInverseConv3d. kernel name is " + kernel_name)
    op_type = "SparseInverseConv3d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__], origin_outputs = [output_img2col_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels)
    result = check_op_cap("op_select_format", "SparseInverseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, indices_in__, unique_indices_offset_in__, sorted_idx_to_former_indices_in__, output_img2col_out_, kernel_size, in_channels)
    result = check_op_cap("get_op_specific_info", "SparseInverseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
