...
def _build_args(boxes_in__, pts_in__, boxes_idx_of_points_out_):
    __inputs__ = []
    for arg in [boxes_in__, pts_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [boxes_idx_of_points_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("PointsInBoxAll", trans_bool_to_s8=False)
...
def points_in_box_all(boxes_in__, pts_in__, boxes_idx_of_points_out_, kernel_name="points_in_box_all", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, pts_in__, boxes_idx_of_points_out_)
    ...
    origin_func_name = "points_in_box_all"
    ascendc_src_dir_ex = "points_in_box_all"
    ascendc_src_dir = "points_in_box_all"
    ascendc_src_file = "points_in_box_all.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator PointsInBoxAll. kernel name is " + kernel_name)
    op_type = "PointsInBoxAll"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[boxes_in__, pts_in__], origin_outputs = [boxes_idx_of_points_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(boxes_in__, pts_in__, boxes_idx_of_points_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, pts_in__, boxes_idx_of_points_out_)
    result = check_op_cap("op_select_format", "PointsInBoxAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(boxes_in__, pts_in__, boxes_idx_of_points_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(boxes_in__, pts_in__, boxes_idx_of_points_out_)
    result = check_op_cap("get_op_specific_info", "PointsInBoxAll", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
