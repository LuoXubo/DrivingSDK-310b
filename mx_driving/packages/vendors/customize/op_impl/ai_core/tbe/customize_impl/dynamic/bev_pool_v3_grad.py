...
def _build_args(grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__, grad_depth_out_, grad_feat_out_, with_depth):
    __inputs__ = []
    for arg in [grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__]:
        if arg != None:
    ...
    if with_depth != None:
        attr = {}
        attr["name"] = "with_depth"
        attr["dtype"] = "bool"
        attr["value"] = with_depth
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("BEVPoolV3Grad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def bev_pool_v3_grad(grad_out_in__, depth_in__=None, feat_in__=None, ranks_depth_in__=None, ranks_feat_in__=None, ranks_bev_in__=None, grad_depth_out_=None, grad_feat_out_=None, with_depth=False, kernel_name="bev_pool_v3_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__, grad_depth_out_, grad_feat_out_, with_depth)
    ...
    origin_func_name = "bev_pool_v3_grad"
    ascendc_src_dir_ex = "bev_pool_v3_grad"
    ascendc_src_dir = "b_e_v_pool_v3_grad"
    ascendc_src_file = "bev_pool_v3_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BEVPoolV3Grad. kernel name is " + kernel_name)
    op_type = "BEVPoolV3Grad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__], origin_outputs = [grad_depth_out_, grad_feat_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_out_in__, depth_in__=None, feat_in__=None, ranks_depth_in__=None, ranks_feat_in__=None, ranks_bev_in__=None, grad_depth_out_=None, grad_feat_out_=None, with_depth=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__, grad_depth_out_, grad_feat_out_, with_depth)
    result = check_op_cap("op_select_format", "BEVPoolV3Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_out_in__, depth_in__=None, feat_in__=None, ranks_depth_in__=None, ranks_feat_in__=None, ranks_bev_in__=None, grad_depth_out_=None, grad_feat_out_=None, with_depth=False, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_out_in__, depth_in__, feat_in__, ranks_depth_in__, ranks_feat_in__, ranks_bev_in__, grad_depth_out_, grad_feat_out_, with_depth)
    result = check_op_cap("get_op_specific_info", "BEVPoolV3Grad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
