...
def _build_args(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type):
    __inputs__ = []
    for arg in [feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [voxel_feats_out_, compare_mask_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("DynamicScatter", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_STR, para_check.KERNEL_NAME)
def dynamic_scatter(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type, kernel_name="dynamic_scatter", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type)
    ...
    origin_func_name = "dynamic_scatter"
    ascendc_src_dir_ex = "dynamic_scatter"
    ascendc_src_dir = "dynamic_scatter"
    ascendc_src_file = "dynamic_scatter.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DynamicScatter. kernel name is " + kernel_name)
    op_type = "DynamicScatter"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__], origin_outputs = [voxel_feats_out_, compare_mask_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type)
    result = check_op_cap("op_select_format", "DynamicScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feats_in__, prefix_sum_point_per_voxel_in__, argsort_coor_in__, voxel_feats_out_, compare_mask_out_, reduce_type)
    result = check_op_cap("get_op_specific_info", "DynamicScatter", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
