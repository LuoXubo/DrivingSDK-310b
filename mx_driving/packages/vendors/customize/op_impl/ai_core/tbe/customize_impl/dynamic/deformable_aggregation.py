...
def _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups):
    __inputs__ = []
    for arg in [mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__]:
        if arg != None:
    ...
    if num_embeds != None:
        attr = {}
        attr["name"] = "num_embeds"
        attr["dtype"] = "int"
        attr["value"] = num_embeds
        __attrs__.append(attr)
    if num_anchors != None:
        attr = {}
        attr["name"] = "num_anchors"
        attr["dtype"] = "int"
        attr["value"] = num_anchors
        __attrs__.append(attr)
    if num_pts != None:
        attr = {}
        attr["name"] = "num_pts"
    ...
    if num_scale != None:
        ...
        attr["name"] = "num_scale"
        attr["dtype"] = "int"
        attr["value"] = num_scale
        __attrs__.append(attr)
    if num_groups != None:
        attr = {}
        attr["name"] = "num_groups"
        attr["dtype"] = "int"
        attr["value"] = num_groups
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("DeformableAggregation", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def deformable_aggregation(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups, kernel_name="deformable_aggregation", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups)
    ...
    origin_func_name = "deformable_aggregation"
    ascendc_src_dir_ex = "deformable_aggregation"
    ascendc_src_dir = "deformable_aggregation"
    ascendc_src_file = "deformable_aggregation.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DeformableAggregation. kernel name is " + kernel_name)
    op_type = "DeformableAggregation"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups)
    result = check_op_cap("op_select_format", "DeformableAggregation", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(mc_ms_feat_in__, spatial_shape_in__, scale_start_index_in__, sampling_location_in__, weights_in__, out_out_, batch_size, num_feat, num_embeds, num_anchors, num_pts, num_cams, num_scale, num_groups)
    result = check_op_cap("get_op_specific_info", "DeformableAggregation", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
