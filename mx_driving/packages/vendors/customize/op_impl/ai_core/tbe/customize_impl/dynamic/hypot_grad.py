...
def _build_args(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_):
    __inputs__ = []
    for arg in [x_in__, y_in__, z_in__, z_grad_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [x_grad_out_, y_grad_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("HypotGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def hypot_grad(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_, kernel_name="hypot_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_)
    ...
    origin_func_name = "hypot_grad"
    ascendc_src_dir_ex = "hypot_grad"
    ascendc_src_dir = "hypot_grad"
    ascendc_src_file = "hypot_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator HypotGrad. kernel name is " + kernel_name)
    op_type = "HypotGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, y_in__, z_in__, z_grad_in__], origin_outputs = [x_grad_out_, y_grad_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_)
    result = check_op_cap("op_select_format", "HypotGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, y_in__, z_in__, z_grad_in__, x_grad_out_, y_grad_out_)
    result = check_op_cap("get_op_specific_info", "HypotGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
