...
def _build_args(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel):
    __inputs__ = []
    for arg in [pts_idx_of_voxels_in__, grad_out_in__]:
        if arg != None:
    ...
    if npoints != None:
        ...
        attr["value"] = npoints
        __attrs__.append(attr)
    if max_pts_per_voxel != None:
        attr = {}
        attr["name"] = "max_pts_per_voxel"
        attr["dtype"] = "int"
        attr["value"] = max_pts_per_voxel
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("RoiawareAvgpool3dGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def roiaware_avgpool3d_grad(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel, kernel_name="roiaware_avgpool3d_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel)
    ...
    origin_func_name = "roiaware_avgpool3d_grad"
    ascendc_src_dir_ex = "roiaware_avgpool3d_grad"
    ascendc_src_dir = "roiaware_avgpool3d_grad"
    ascendc_src_file = "roiaware_avgpool3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RoiawareAvgpool3dGrad. kernel name is " + kernel_name)
    op_type = "RoiawareAvgpool3dGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[pts_idx_of_voxels_in__, grad_out_in__], origin_outputs = [grad_in_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel)
    result = check_op_cap("op_select_format", "RoiawareAvgpool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(pts_idx_of_voxels_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, max_pts_per_voxel)
    result = check_op_cap("get_op_specific_info", "RoiawareAvgpool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
