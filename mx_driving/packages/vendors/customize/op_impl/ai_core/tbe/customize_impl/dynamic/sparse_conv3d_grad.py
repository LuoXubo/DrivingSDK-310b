...
def _build_args(features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__, features_grad_out_, weight_grad_out_, start_offset, end_offset):
    __inputs__ = []
    for arg in [features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [features_grad_out_, weight_grad_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if start_offset != None:
        attr = {}
        attr["name"] = "start_offset"
        attr["dtype"] = "int"
        attr["value"] = start_offset
        __attrs__.append(attr)
    if end_offset != None:
        attr = {}
        attr["name"] = "end_offset"
        attr["dtype"] = "int"
        attr["value"] = end_offset
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("SparseConv3dGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.OPTION_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def sparse_conv3d_grad(features_in__, weight_in__, grad_out_features_in__=None, former_sorted_indices_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, start_offset=0, end_offset=0, kernel_name="sparse_conv3d_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__, features_grad_out_, weight_grad_out_, start_offset, end_offset)
    ...
    origin_func_name = "sparse_conv3d_grad"
    ascendc_src_dir_ex = "sparse_conv3d_grad"
    ascendc_src_dir = "sparse_conv3d_grad"
    ascendc_src_file = "sparse_conv3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SparseConv3dGrad. kernel name is " + kernel_name)
    op_type = "SparseConv3dGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__], origin_outputs = [features_grad_out_, weight_grad_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(features_in__, weight_in__, grad_out_features_in__=None, former_sorted_indices_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, start_offset=0, end_offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__, features_grad_out_, weight_grad_out_, start_offset, end_offset)
    result = check_op_cap("op_select_format", "SparseConv3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(features_in__, weight_in__, grad_out_features_in__=None, former_sorted_indices_in__=None, indices_offset_in__=None, features_grad_out_=None, weight_grad_out_=None, start_offset=0, end_offset=0, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(features_in__, weight_in__, grad_out_features_in__, former_sorted_indices_in__, indices_offset_in__, features_grad_out_, weight_grad_out_, start_offset, end_offset)
    result = check_op_cap("get_op_specific_info", "SparseConv3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
