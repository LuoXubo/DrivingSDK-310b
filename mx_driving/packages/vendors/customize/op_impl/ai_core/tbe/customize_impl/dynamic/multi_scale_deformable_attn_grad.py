...
def _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_):
    __inputs__ = []
    for arg in [value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("MultiScaleDeformableAttnGrad", trans_bool_to_s8=False)
...
def multi_scale_deformable_attn_grad(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_, kernel_name="multi_scale_deformable_attn_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_)
    ...
    origin_func_name = "multi_scale_deformable_attn_grad"
    ascendc_src_dir_ex = "multi_scale_deformable_attn_grad"
    ascendc_src_dir = "multi_scale_deformable_attn_grad"
    ascendc_src_file = "multi_scale_deformable_attn_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator MultiScaleDeformableAttnGrad. kernel name is " + kernel_name)
    op_type = "MultiScaleDeformableAttnGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__], origin_outputs = [grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_)
    result = check_op_cap("op_select_format", "MultiScaleDeformableAttnGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(value_in__, spatial_shapes_in__, level_start_index_in__, sampling_loc_in__, attn_weight_in__, grad_output_in__, grad_value_out_, grad_sampling_loc_out_, grad_attn_weight_out_)
    result = check_op_cap("get_op_specific_info", "MultiScaleDeformableAttnGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
