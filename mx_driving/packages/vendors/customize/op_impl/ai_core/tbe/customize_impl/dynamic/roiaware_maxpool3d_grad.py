...
def _build_args(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints):
    __inputs__ = []
    for arg in [argmax_in__, grad_out_in__]:
        if arg != None:
...
@tbe_register.register_operator("RoiawareMaxpool3dGrad", trans_bool_to_s8=False)
...
def roiaware_maxpool3d_grad(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, kernel_name="roiaware_maxpool3d_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints)
    ...
    origin_func_name = "roiaware_maxpool3d_grad"
    ascendc_src_dir_ex = "roiaware_maxpool3d_grad"
    ascendc_src_dir = "roiaware_maxpool3d_grad"
    ascendc_src_file = "roiaware_maxpool3d_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator RoiawareMaxpool3dGrad. kernel name is " + kernel_name)
    op_type = "RoiawareMaxpool3dGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[argmax_in__, grad_out_in__], origin_outputs = [grad_in_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints)
    result = check_op_cap("op_select_format", "RoiawareMaxpool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(argmax_in__, grad_out_in__, grad_in_out_, boxes_num, out_x, out_y, out_z, channels, npoints)
    result = check_op_cap("get_op_specific_info", "RoiawareMaxpool3dGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
