...
def _build_args(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k):
    __inputs__ = []
    for arg in [xyz_in__, center_xyz_in__]:
        if arg != None:
    ...
    if is_from_knn != None:
        attr = {}
        attr["name"] = "is_from_knn"
        attr["dtype"] = "bool"
        attr["value"] = is_from_knn
...
@tbe_register.register_operator("Knn", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def knn(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k, kernel_name="knn", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k)
    ...
    origin_func_name = "knn"
    ascendc_src_dir_ex = "knn"
    ascendc_src_dir = "knn"
    ascendc_src_file = "knn.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator Knn. kernel name is " + kernel_name)
    op_type = "Knn"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[xyz_in__, center_xyz_in__], origin_outputs = [dist_out_, idx_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k)
    result = check_op_cap("op_select_format", "Knn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(xyz_in__, center_xyz_in__, dist_out_, idx_out_, is_from_knn, k)
    result = check_op_cap("get_op_specific_info", "Knn", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
