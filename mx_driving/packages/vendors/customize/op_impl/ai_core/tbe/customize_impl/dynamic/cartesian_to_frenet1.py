...
def _build_args(dist_vec_in__, min_idx_out_, back_idx_out_):
    __inputs__ = []
    for arg in [dist_vec_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [min_idx_out_, back_idx_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("CartesianToFrenet1", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.KERNEL_NAME)
def cartesian_to_frenet1(dist_vec_in__, min_idx_out_, back_idx_out_, kernel_name="cartesian_to_frenet1", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(dist_vec_in__, min_idx_out_, back_idx_out_)
    ...
    origin_func_name = "cartesian_to_frenet1"
    ascendc_src_dir_ex = "cartesian_to_frenet1"
    ascendc_src_dir = "cartesian_to_frenet1"
    ascendc_src_file = "cartesian_to_frenet1.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator CartesianToFrenet1. kernel name is " + kernel_name)
    op_type = "CartesianToFrenet1"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[dist_vec_in__], origin_outputs = [min_idx_out_, back_idx_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(dist_vec_in__, min_idx_out_, back_idx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dist_vec_in__, min_idx_out_, back_idx_out_)
    result = check_op_cap("op_select_format", "CartesianToFrenet1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(dist_vec_in__, min_idx_out_, back_idx_out_, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(dist_vec_in__, min_idx_out_, back_idx_out_)
    result = check_op_cap("get_op_specific_info", "CartesianToFrenet1", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
