...
def _build_args(src_in__, indices_in__, var_in__, out_out_, count_out_, dim):
    __inputs__ = []
    for arg in [src_in__, indices_in__, var_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [out_out_, count_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("ScatterMean", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def scatter_mean(src_in__, indices_in__, var_in__, out_out_, count_out_, dim, kernel_name="scatter_mean", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, count_out_, dim)
    ...
    origin_func_name = "scatter_mean"
    ascendc_src_dir_ex = "scatter_mean"
    ascendc_src_dir = "scatter_mean"
    ascendc_src_file = "scatter_mean.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator ScatterMean. kernel name is " + kernel_name)
    op_type = "ScatterMean"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[src_in__, indices_in__, var_in__], origin_outputs = [out_out_, count_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(src_in__, indices_in__, var_in__, out_out_, count_out_, dim, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, count_out_, dim)
    result = check_op_cap("op_select_format", "ScatterMean", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(src_in__, indices_in__, var_in__, out_out_, count_out_, dim, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(src_in__, indices_in__, var_in__, out_out_, count_out_, dim)
    result = check_op_cap("get_op_specific_info", "ScatterMean", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
