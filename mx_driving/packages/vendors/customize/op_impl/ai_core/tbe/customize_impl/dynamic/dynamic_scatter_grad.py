...
def _build_args(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type):
    __inputs__ = []
    for arg in [grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__]:
        if arg != None:
    ...
    if reduce_type != None:
        attr = {}
        attr["name"] = "reduce_type"
        attr["dtype"] = "str"
        attr["value"] = reduce_type
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("DynamicScatterGrad", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_STR, para_check.KERNEL_NAME)
def dynamic_scatter_grad(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type, kernel_name="dynamic_scatter_grad", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type)
    ...
    origin_func_name = "dynamic_scatter_grad"
    ascendc_src_dir_ex = "dynamic_scatter_grad"
    ascendc_src_dir = "dynamic_scatter_grad"
    ascendc_src_file = "dynamic_scatter_grad.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator DynamicScatterGrad. kernel name is " + kernel_name)
    op_type = "DynamicScatterGrad"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__], origin_outputs = [grad_feats_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type)
    result = check_op_cap("op_select_format", "DynamicScatterGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(grad_voxel_feats_in__, num_point_per_voxel_in__, argsort_coor_in__, compare_mask_in__, grad_feats_out_, reduce_type)
    result = check_op_cap("get_op_specific_info", "DynamicScatterGrad", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
