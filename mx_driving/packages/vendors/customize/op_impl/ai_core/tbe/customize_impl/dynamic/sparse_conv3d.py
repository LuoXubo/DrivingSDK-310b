...
def _build_args(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding):
    __inputs__ = []
    for arg in [indices_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [indices_out_out_, indices_pair_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
...
@tbe_register.register_operator("SparseConv3d", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.REQUIRED_ATTR_LIST_INT, para_check.KERNEL_NAME)
def sparse_conv3d(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding, kernel_name="sparse_conv3d", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding)
    ...
    origin_func_name = "sparse_conv3d"
    ascendc_src_dir_ex = "sparse_conv3d"
    ascendc_src_dir = "sparse_conv3d"
    ascendc_src_file = "sparse_conv3d.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator SparseConv3d. kernel name is " + kernel_name)
    op_type = "SparseConv3d"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[indices_in__], origin_outputs = [indices_out_out_, indices_pair_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding)
    result = check_op_cap("op_select_format", "SparseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(indices_in__, indices_out_out_, indices_pair_out_, kernel_size, out_spatial_shape, stride, padding)
    result = check_op_cap("get_op_specific_info", "SparseConv3d", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
