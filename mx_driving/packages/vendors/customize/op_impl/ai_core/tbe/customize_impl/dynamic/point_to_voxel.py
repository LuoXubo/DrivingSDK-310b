...
def _build_args(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout):
    __inputs__ = []
    ...
    if voxel_sizes != None:
        attr = {}
        attr["name"] = "voxel_sizes"
        attr["dtype"] = "list_float"
        attr["value"] = voxel_sizes
        __attrs__.append(attr)
    if coor_ranges != None:
        attr = {}
        attr["name"] = "coor_ranges"
        attr["dtype"] = "list_float"
        attr["value"] = coor_ranges
        __attrs__.append(attr)
    if layout != None:
        attr = {}
        attr["name"] = "layout"
...
@tbe_register.register_operator("PointToVoxel", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_LIST_FLOAT, para_check.REQUIRED_ATTR_LIST_FLOAT, para_check.REQUIRED_ATTR_STR, para_check.KERNEL_NAME)
def point_to_voxel(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout, kernel_name="point_to_voxel", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout)
    ...
    origin_func_name = "point_to_voxel"
    ascendc_src_dir_ex = "point_to_voxel"
    ascendc_src_dir = "point_to_voxel"
    ascendc_src_file = "point_to_voxel.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator PointToVoxel. kernel name is " + kernel_name)
    op_type = "PointToVoxel"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[points_in__], origin_outputs = [voxels_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout)
    result = check_op_cap("op_select_format", "PointToVoxel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(points_in__, voxels_out_, voxel_sizes, coor_ranges, layout)
    result = check_op_cap("get_op_specific_info", "PointToVoxel", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
