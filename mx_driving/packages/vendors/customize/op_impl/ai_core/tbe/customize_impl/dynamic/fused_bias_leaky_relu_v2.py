...
def _build_args(x_in__, bias_in__, output_out_, negative_slope, scale):
    __inputs__ = []
...
@tbe_register.register_operator("FusedBiasLeakyReluV2", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_FLOAT, para_check.REQUIRED_ATTR_FLOAT, para_check.KERNEL_NAME)
def fused_bias_leaky_relu_v2(x_in__, bias_in__, output_out_, negative_slope, scale, kernel_name="fused_bias_leaky_relu_v2", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, output_out_, negative_slope, scale)
    ...
    origin_func_name = "fused_bias_leaky_relu_v2"
    ascendc_src_dir_ex = "fused_bias_leaky_relu_v2"
    ascendc_src_dir = "fused_bias_leaky_relu_v2"
    ascendc_src_file = "fused_bias_leaky_relu_v2.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FusedBiasLeakyReluV2. kernel name is " + kernel_name)
    op_type = "FusedBiasLeakyReluV2"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[x_in__, bias_in__], origin_outputs = [output_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(x_in__, bias_in__, output_out_, negative_slope, scale, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, output_out_, negative_slope, scale)
    result = check_op_cap("op_select_format", "FusedBiasLeakyReluV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(x_in__, bias_in__, output_out_, negative_slope, scale, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(x_in__, bias_in__, output_out_, negative_slope, scale)
    result = check_op_cap("get_op_specific_info", "FusedBiasLeakyReluV2", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
