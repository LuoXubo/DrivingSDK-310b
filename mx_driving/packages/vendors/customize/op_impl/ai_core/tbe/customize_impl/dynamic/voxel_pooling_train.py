...
def _build_args(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z):
    __inputs__ = []
    for arg in [geom_in__, input_features_in__]:
        if arg != None:
    ...
    __outputs__ = []
    for arg in [output_features_out_, pos_memo_out_]:
        if arg != None:
            if isinstance(arg, (list, tuple)):
                if len(arg) == 0:
                    continue
    ...
    if num_channels != None:
        attr = {}
        attr["name"] = "num_channels"
        attr["dtype"] = "int"
        attr["value"] = num_channels
        __attrs__.append(attr)
    if num_voxel_x != None:
        attr = {}
        attr["name"] = "num_voxel_x"
        attr["dtype"] = "int"
        attr["value"] = num_voxel_x
        __attrs__.append(attr)
    if num_voxel_y != None:
        attr = {}
        attr["name"] = "num_voxel_y"
        attr["dtype"] = "int"
        attr["value"] = num_voxel_y
        __attrs__.append(attr)
    if num_voxel_z != None:
        attr = {}
        attr["name"] = "num_voxel_z"
        attr["dtype"] = "int"
        attr["value"] = num_voxel_z
        __attrs__.append(attr)
    return __inputs__, __outputs__, __attrs__

@tbe_register.register_operator("VoxelPoolingTrain", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_INT, para_check.KERNEL_NAME)
def voxel_pooling_train(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z, kernel_name="voxel_pooling_train", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z)
    ...
    origin_func_name = "voxel_pooling_train"
    ascendc_src_dir_ex = "voxel_pooling_train"
    ascendc_src_dir = "voxel_pooling_train"
    ascendc_src_file = "voxel_pooling_train.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator VoxelPoolingTrain. kernel name is " + kernel_name)
    op_type = "VoxelPoolingTrain"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[geom_in__, input_features_in__], origin_outputs = [output_features_out_, pos_memo_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z)
    result = check_op_cap("op_select_format", "VoxelPoolingTrain", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(geom_in__, input_features_in__, output_features_out_, pos_memo_out_, batch_size, num_points, num_channels, num_voxel_x, num_voxel_y, num_voxel_z)
    result = check_op_cap("get_op_specific_info", "VoxelPoolingTrain", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
