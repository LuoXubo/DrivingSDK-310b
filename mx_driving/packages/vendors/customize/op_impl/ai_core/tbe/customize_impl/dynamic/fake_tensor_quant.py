...
def _build_args(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range):
    __inputs__ = []
...
@tbe_register.register_operator("FakeTensorQuant", trans_bool_to_s8=False)
@para_check.check_op_params(para_check.REQUIRED_INPUT, para_check.REQUIRED_INPUT, para_check.REQUIRED_OUTPUT, para_check.REQUIRED_ATTR_INT, para_check.REQUIRED_ATTR_BOOL, para_check.REQUIRED_ATTR_BOOL, para_check.KERNEL_NAME)
def fake_tensor_quant(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range, kernel_name="fake_tensor_quant", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range)
    ...
    origin_func_name = "fake_tensor_quant"
    ascendc_src_dir_ex = "fake_tensor_quant"
    ascendc_src_dir = "fake_tensor_quant"
    ascendc_src_file = "fake_tensor_quant.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator FakeTensorQuant. kernel name is " + kernel_name)
    op_type = "FakeTensorQuant"
...
def op_select_format(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range)
    result = check_op_cap("op_select_format", "FakeTensorQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(inputs_in__, amax_in__, out_out_, num_bits, is_unsigned, narrow_range)
    result = check_op_cap("get_op_specific_info", "FakeTensorQuant", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
