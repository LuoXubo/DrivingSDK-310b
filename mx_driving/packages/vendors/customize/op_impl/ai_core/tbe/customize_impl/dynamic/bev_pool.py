...
def _build_args(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c):
    __inputs__ = []
    for arg in [feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__]:
        if arg != None:
...
@tbe_register.register_operator("BEVPool", trans_bool_to_s8=False)
...
def bev_pool(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c, kernel_name="bev_pool", impl_mode=""):
    if get_current_build_config("enable_op_prebuild"):
        return
    __inputs__, __outputs__, __attrs__ = _build_args(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c)
    ...
    origin_func_name = "bev_pool"
    ascendc_src_dir_ex = "bev_pool"
    ascendc_src_dir = "b_e_v_pool"
    ascendc_src_file = "bev_pool.cpp"
    src = get_kernel_source(ascendc_src_file, ascendc_src_dir, ascendc_src_dir_ex)

    print("start compile Ascend C operator BEVPool. kernel name is " + kernel_name)
    op_type = "BEVPool"
    ...
    op_info = OpInfo(kernel_name = kernel_name, op_type = op_type, inputs = __inputs__, outputs = __outputs__,\
        attrs = __attrs__, impl_mode = impl_mode, origin_inputs=[feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__], origin_outputs = [out_out_])
    compile_op(src, origin_func_name, op_info, options, code_channel, '{}')

def op_select_format(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c)
    result = check_op_cap("op_select_format", "BEVPool", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")

def get_op_specific_info(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c, impl_mode=""):
    __inputs__, __outputs__, __attrs__ = _build_args(feat_in__, geom_feat_in__, interval_lengths_in__, interval_starts_in__, out_out_, b, d, h, w, c)
    result = check_op_cap("get_op_specific_info", "BEVPool", __inputs__, __outputs__, __attrs__)
    return result.decode("utf-8")
