...
class BEVDetOCC(BEVDet):
    def __init__(self,
        ...
        # LiDAR BEV fusion: grid config matches camera BEV
        self.lidar_grid_x = [-40.0, 40.0, 0.4]  # 200 cells
        self.lidar_grid_y = [-40.0, 40.0, 0.4]  # 200 cells
        self.lidar_bev_conv = torch.nn.Sequential(
            torch.nn.Conv2d(5, 64, kernel_size=3, padding=1),
            torch.nn.BatchNorm2d(64),
            torch.nn.ReLU(inplace=True),
            torch.nn.Conv2d(64, 256, kernel_size=3, padding=1),
        )

    def lidar_to_bev(self, points):
        """Convert LiDAR points to multi-channel BEV feature grid.

        Args:
            points: list of (N_i, 5) tensors [x, y, z, intensity, time]

        Returns:
            bev: (B, 5, 200, 200) grid with [occupancy, mean_z, mean_intensity,
                 mean_x_centered, mean_y_centered] per cell
        """
        B = len(points)
        device = points[0].device
        gx_min, gx_max, gx_step = self.lidar_grid_x
        gy_min, gy_max, gy_step = self.lidar_grid_y
        H = int((gy_max - gy_min) / gy_step)  # 200
        W = int((gx_max - gx_min) / gx_step)  # 200
        ...
        for b in range(B):
            ...
            if pts.numel() == 0:
                continue
            xs = ((pts[:, 0] - gx_min) / gx_step).long()
            ys = ((pts[:, 1] - gy_min) / gy_step).long()
            ...
            pts_v = pts[valid]
            # occupancy
            bev[b, 0, ys_v, xs_v] += 1.0
            # mean z
            bev[b, 1, ys_v, xs_v] += pts_v[:, 2]
            # mean intensity
            bev[b, 2, ys_v, xs_v] += pts_v[:, 3]
            # centered x, y relative to cell center
            cell_cx = (xs_v.float() + 0.5) * gx_step + gx_min
            cell_cy = (ys_v.float() + 0.5) * gy_step + gy_min
            bev[b, 3, ys_v, xs_v] += pts_v[:, 0] - cell_cx
            bev[b, 4, ys_v, xs_v] += pts_v[:, 1] - cell_cy

        # normalise by point count per cell
        counts = bev[:, 0:1, :, :].clamp(min=1.0)
        bev[:, 1:, :, :] = bev[:, 1:, :, :] / counts
        # log-scale occupancy to tame range
        bev[:, 0:1, :, :] = torch.log1p(bev[:, 0:1, :, :])
    ...
    def forward_train(self,
        ...
        # Camera-LiDAR fusion: add LiDAR BEV features to camera BEV features
        if points is not None and len(points) > 0 and points[0].numel() > 0:
            lidar_bev = self.lidar_to_bev(points)         # (B, 5, 200, 200)
            lidar_feat = self.lidar_bev_conv(lidar_bev)   # (B, 256, 200, 200)
            occ_bev_feature = occ_bev_feature + lidar_feat
...
def _trt_reshape_part1_outputs(vt, tran_feat, depth, num_cam=6):
    """Match LSS frustum layout (D, fH, fW), not legacy hard-coded (16, 44, 44)."""
    d = int(vt.D)
    fh = int(vt.frustum.shape[1])
    fw = int(vt.frustum.shape[2])
    c = int(vt.out_channels)
    tran_feat = tran_feat.reshape(num_cam, fh, fw, c)
    depth = depth.reshape(num_cam, d, fh, fw)
    return tran_feat, depth


def _trt_reshape_part3_input(vt, x):
    c = int(vt.out_channels)
    dy = int(vt.grid_size[1])
    dx = int(vt.grid_size[0])
    return x.reshape(1, dy, dx, c)
...
class BEVDetOCCTRT(BEVDetOCC):
    ...
    def forward_part2(
        ...
        tran_feat, depth = _trt_reshape_part1_outputs(vt, tran_feat, depth)
    ...
    def forward_part3(
        ...
        x = _trt_reshape_part3_input(self.img_view_transformer, x)
