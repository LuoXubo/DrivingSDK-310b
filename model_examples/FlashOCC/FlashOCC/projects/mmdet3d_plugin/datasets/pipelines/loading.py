...
class PrepareImageInputs(object):
    ...
    def depth_transform_core(self, depth, resize_dims, crop, flip, rotate):
        import cv2
        d = depth.astype(np.float32)
        d[d >= self.depth_invalid] = 0.0
        d = cv2.resize(d, resize_dims, interpolation=cv2.INTER_NEAREST)
        ...
        if abs(rotate) > 1e-3:
            ...
            d = cv2.warpAffine(d, m, (w, h), flags=cv2.INTER_NEAREST, borderValue=0)
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        canvas = []
        depth_maps = [] if self.load_depth else None
        ...
        for cam_name in cam_names:
            ...
            if self.load_depth:
                depth_path = cam_data.get('depth_path')
                if depth_path and os.path.isfile(depth_path):
                    depth_np = np.load(depth_path)
                    depth_t = self.depth_transform_core(
                        depth_np, resize_dims, crop, flip, rotate)
                    depth_maps.append(depth_t)
                else:
                    h = int(crop[3] - crop[1])
                    w = int(crop[2] - crop[0])
                    depth_maps.append(torch.zeros((h, w), dtype=torch.float32))
            ...
            if self.sequential:
                ...
                for adj_info in results['adjacent']:
                    ...
                    if img_adjacent.mode != 'RGB':
                        img_adjacent = img_adjacent.convert('RGB')
        ...
        if self.load_depth and depth_maps is not None:
...
@PIPELINES.register_module()
class LoadDepthGTFromFile(object):
    """Load per-camera depth maps (meters) from paths in cam info."""

    def __init__(self, invalid_value=60000.0, downsample=1):
    ...
    def __call__(self, results):
        if 'gt_depth' in results:
            return results
        import cv2
        imgs, _, _, intrins = results['img_inputs'][:4]
        ...
        for cid, cam_name in enumerate(cam_names):
            ...
            if depth_path is None:
                raise KeyError(f'depth_path missing for {cam_name}')
            depth = np.load(depth_path).astype(np.float32)
            depth[depth >= self.invalid_value] = 0.0
            depth_t = torch.from_numpy(depth)
            # Apply same 2D aug as images (post_rot/post_trans are 3x3/3)
            # Approximate: resize/crop already reflected in img; use raw depth
            ...
            if depth_t.shape[0] != h or depth_t.shape[1] != w:
                depth_t = torch.nn.functional.interpolate(
                    depth_t[None, None], size=(h, w), mode='nearest')[0, 0]
            if self.downsample > 1:
                depth_t = depth_t[::self.downsample, ::self.downsample]
            depth_map_list.append(depth_t)
